        ...
        public override bool Setup()
        {
            try
            {
               ...
               arrivalDistanceThreshold = 15f;
                ...
                AmbientAICallouts.Helper.LogLSPDFRAPIfunction("GetPersonaForPed()");
                        ...
                        GameFiber.StartNew(delegate
                        {
                            try
                            {
                                ...
                                {                                                                                   //sollange call läuft //Workaround
                                    AmbientAICallouts.Helper.LogLSPDFRAPIfunction("IsPedArrested(), IsPedStoppedByPlayer()");
                        ...
                        foreach (var officer in Units[0].UnitOfficers) 
                        {
                            ...
                            GameFiber.StartNew(delegate
                            {
                                ...
                                while (task == 2)
                                {
                                    if (officer)
                                    {
                                        AmbientAICallouts.Helper.LogLSPDFRAPIfunction("IsPedGettingArrested(), IsPedArrested()");
                                        ...
                                        {
                                            try {
                                                AmbientAICallouts.Helper.LogLSPDFRAPIfunction("SetCopIgnoreAmbientCombatControl()");
